# Testing tool for Yunex OBU and RSU
...
parser = argparse.ArgumentParser(description="Testing tool for Yunex OBU and RSU")
...
results = all_devs.sudo("ip link set atk-cv2x-tx up", in_stream=False)
...
for i in range(args.repeat):
    ...
    filename = f"yun_tx_{timestamp}.pcap"
    try:
        results = all_devs.sudo(
            f"tcpdump -i atk-cv2x-tx -U -w /tmp/{filename}",
            hide=True,
            in_stream=False,
